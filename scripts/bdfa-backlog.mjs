...
function fail(message) {
  console.error(`BDFA backlog error: ${message}`);
  process.exit(1);
}
...
function parseMilestones(markdown) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const recommendedLevel = levelLine.replace(/^\*\*Recommended level:\*\*\s*/, '').trim();
    milestones.push({
...
function writeOutput(name, value) {
  ...
  const delimiter = `BDFA_${name.toUpperCase()}_${Date.now()}_${Math.random().toString(16).slice(2)}`;
...
if (args.command === 'validate') {
  ...
  console.log(`Validated ${milestones.length} BDFA milestones.`);
  process.exit(0);
}
